...
function cmsms_woocommerce_header_cart_link() {
	echo '<a href="' . esc_url(wc_get_cart_url()) . '" class="cmsms_header_cart_link cmsms_theme_icon_basket"></a>';
}
...
function cmsms_woocommerce_rating($icon_trans = '', $icon_color = '', $in_review = false, $comment_id = '', $show = true) {
	...
	$out = "
<div class=\"cmsms_star_rating\" itemprop=\"{$itemprop}\" itemscope itemtype=\"http://schema.org/{$itemtype}\" title=\"" . sprintf(esc_html__('Rated %s out of 5', 'pet-rescue'), $rating) . "\">
...
<span class=\"rating dn\"><strong itemprop=\"ratingValue\">" . esc_html($rating) . "</strong> " . esc_html__('out of 5', 'pet-rescue') . "</span>
</div>
";
