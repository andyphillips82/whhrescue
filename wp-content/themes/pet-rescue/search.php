		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('cmsms_search_post'); ?>>
			...
			<div class="cmsms_search_post_cont">
				...
				<header class="cmsms_search_post_header entry-header">
					<h3 class="cmsms_search_post_title entry-title">
				...
				if (
					...
					echo '<div class="cmsms_search_post_cont_info entry-meta">' . 
						'<span class="cmsms_search_post_user_name">' . 
							...
							'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" rel="author" title="' . esc_attr__('Posts by', 'pet-rescue') . ' ' . get_the_author_meta('display_name') . '">' . get_the_author_meta('display_name') . '</a>' . 
						'</span>';
				...
				<footer class="cmsms_search_post_footer entry-meta">
					...
					<?php 
					if (comments_open()) {
						echo '<a class="cmsms_search_post_comments cmsms_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'pet-rescue') . ' ' . get_the_title() . '">' . get_comments_number() . '</a>';
					}
					?>
					</div>
					<?php 
					echo '<a class="button cmsms_search_post_read_more" href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'pet-rescue') . '</a>';
					?>
				</footer>
			</div>
		</article>
