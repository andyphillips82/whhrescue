...
function register_css_styles() {
	if (!is_admin()) {
		...
		if (shortcode_exists('cmsms_portfolio')) {
...
function register_js_scripts() {
	if (!is_admin()) {
		...
		wp_localize_script('script', 'cmsms_script', array( 
			...
			'ilightbox_close_text' => 				__('Close', 'pet-rescue'), 
			'ilightbox_enter_fullscreen_text' => 	__('Enter Fullscreen (Shift+Enter)', 'pet-rescue'), 
			'ilightbox_exit_fullscreen_text' => 	__('Exit Fullscreen (Shift+Enter)', 'pet-rescue'), 
			'ilightbox_slideshow_text' => 			__('Slideshow', 'pet-rescue'), 
			'ilightbox_next_text' => 				__('Next', 'pet-rescue'), 
			'ilightbox_previous_text' => 			__('Previous', 'pet-rescue'), 
			'ilightbox_load_image_error' => 		__('An error occurred when trying to load photo.', 'pet-rescue'), 
			'ilightbox_load_contents_error' => 		__('An error occurred when trying to load contents.', 'pet-rescue'), 
			'ilightbox_missing_plugin_error' => 	__("The content your are attempting to view requires the <a href='{pluginspage}' target='_blank'>{type} plugin<\/a>.", 'pet-rescue') 
		));
...
function the_widgets_init() {
    ...
    register_sidebar(
        array(
            'name' => __('Sidebar', 'pet-rescue'), 
            'id' => 'sidebar_default', 
            'description' => __('Widgets in this area will be shown in all left and right sidebars till you don\'t use custom sidebar.', 'pet-rescue'), 
    ...
    register_sidebar(
        array(
            'name' => __('Bottom Sidebar', 'pet-rescue'), 
            'id' => 'sidebar_bottom', 
            'description' => __('Widgets in this area will be shown at the bottom of middle block below the content and middle sidebar, but above footer.', 'pet-rescue'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h5 class="widgettitle">', 
            'after_title' => '</h5>'
	...
	if (isset($cmsms_option[CMSMS_SHORTNAME . '_sidebar']) && sizeof($cmsms_option[CMSMS_SHORTNAME . '_sidebar']) > 0) {
		foreach ($cmsms_option[CMSMS_SHORTNAME . '_sidebar'] as $sidebar) {
			register_sidebar(array( 
				...
				'description' => __('Custom sidebar created with cmsmasters admin panel.', 'pet-rescue'), 
				'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
				'after_widget' => '</aside>', 
				'before_title' => '<h5 class="widgettitle">', 
				'after_title' => '</h5>' 
...
register_nav_menus(array( 
    'primary' => 	__('Primary Navigation', 'pet-rescue'), 
    'footer' => 	__('Footer Navigation', 'pet-rescue'), 
	'top_line' => 	__('Top Line Navigation', 'pet-rescue') 
));
...
function sidebarDefaultText() {
	...
	'<aside id="archives" class="widget widget_archive">' . "\n" . 
		'<h3 class="widgettitle">' . esc_html__('Archives', 'pet-rescue') . '</h3>' . "\n" . 
		'<ul>' . "\n";
	...
	'<aside id="meta" class="widget widget_meta">' . "\n" . 
		'<h3 class="widgettitle">' . esc_html__('Meta', 'pet-rescue') . '</h3>' . "\n" . 
		'<ul>' . "\n\t";
	...
	echo '" title="' . esc_attr__('Syndicate this site using RSS 2.0', 'pet-rescue') . '">' . esc_html__('Entries', 'pet-rescue') . ' ' . 
			'<abbr title="' . esc_attr__('Really Simple Syndication', 'pet-rescue') . '">' . esc_html__('RSS', 'pet-rescue') . '</abbr>' . 
		'</a>' . 
	'</li>' . "\n\t" . 
	'<li>' . 
		'<a href="';
	...
	echo '" title="' . esc_attr__('The latest comments to all posts in RSS', 'pet-rescue') . '">' . esc_html__('Comments', 'pet-rescue') . ' ' . 
					'<abbr title="' . esc_attr__('Really Simple Syndication', 'pet-rescue') . '">' . esc_html__('RSS', 'pet-rescue') . '</abbr>' . 
				'</a>' . 
			'</li>' . "\n\t" . 
			'<li>' . 
				'<a href="http://wordpress.org/" title="' . esc_attr__('Powered by WordPress, state-of-the-art semantic personal publishing platform.', 'pet-rescue') . '">WordPress.org</a>' . 
			'</li>' . "\r" . 
		'</ul>' . "\n" . 
	'</aside>' . "\n";
}
