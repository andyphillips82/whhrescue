 ...
 * @subpackage 	cmsms Content Composer
 * @version		1.5.7
 * 
...
class Cmsms_Gutenberg {
	protected $is_gutenberg_editor_active = false;

	public function __construct() {
		add_action( 'rest_api_init', array( $this, 'register_cmsms_rest_field' ) );
		add_action( 'enqueue_block_editor_assets', array( $this, 'enqueue_assets' ) );
		add_action( 'admin_footer', array( $this, 'print_admin_js_template' ) );
	}

	public function register_cmsms_rest_field() {
		...
		register_rest_field( get_post_types( '', 'names' ), 'gutenberg_cmsms_composer_mode', $rest_field_args );
	}
	...
	public function enqueue_assets() {
		...
		wp_enqueue_style( 'cmsms_composer_gutenberg_css', CMSMS_CONTENT_COMPOSER_URL . 'gutenberg/css/gutenberg.css', array(), CMSMS_CONTENT_COMPOSER_VERSION, 'screen' );

		wp_enqueue_script( 'cmsms_composer_gutenberg_js', CMSMS_CONTENT_COMPOSER_URL . 'gutenberg/js/gutenberg.js', array( 'jquery' ), CMSMS_CONTENT_COMPOSER_VERSION, true );

		wp_localize_script( 'cmsms_composer_gutenberg_js', 'cmsms_gutenberg', array(
			'temp_title' => 		__( 'Temporary title', 'cmsms_content_composer' ),
		...
		<script id="composer-gutenberg-button-switch-mode" type="text/html">
			<div id="composer-switch-mode">
				<button id="composer-switch-mode-button" type="button" class="button button-primary button-large admin-icon-composer">
					<span class="composer-switch-mode-on"><?php _e( '&#8592; Back to WordPress Editor', 'cmsms_content_composer' ); ?></span>
					<span class="composer-switch-mode-off">
						<?php _e( 'Content Composer', 'cmsms_content_composer' ); ?>
					</span>
				</button>
			</div>
		</script>
			...
			<div id="composer-editor">
				<a id="composer-go-to-edit-page-link" href="#">
					...
					<div class="composer-loader-wrapper">
						...
						<div class="composer-loading-title"><?php _e( 'Loading...', 'cmsms_content_composer' ); ?></div>
					</div>
				</a>
			</div>
		</script>
	...
	public function set_is_composer_page( $post_id, $is_composer = true ) {
		if ( $is_composer ) {
			update_post_meta( $post_id, 'cmsms_composer_show', true );
		} else {
			delete_post_meta( $post_id, 'cmsms_composer_show' );
		}
	}

	public function is_built_with_composer( $post_id ) {
		return get_post_meta( $post_id, 'cmsms_composer_show', true );
	}
