		...
		obj.methods = { 
			...
			startObj : function () { 
				...
				obj.methods.el.droppable( { 
					...
					greedy : true, 
					tolerance : 'fit', 
					hoverClass : 'cmsmsComposerHover', 
					deactivate : function (event, ui) { 
						$(event.target).removeClass('cmsmsComposerHover');
					}, 
				...
				obj.methods.el.sortable( { 
					...
					start : function (event, ui) { 
						var placeClasses = ui.item.context.className.replace(/cmsms_\S+/g, '').replace(/ui-\S+/g, '');
						
						
						$(ui.placeholder).addClass(placeClasses);
						
						
						$(ui.item).addClass('cmsms_move_add');
			...
			makeColumnsSortable : function () { 
				...
				obj.methods.cols.find('> .innerColumn').droppable( { 
					accept : 'a[data-shortcode]:not(a.cmsms_row)', 
					greedy : true, 
					tolerance : 'pointer', 
					hoverClass : 'cmsmsColumnHover', 
					deactivate : function (event, ui) { 
						$(event.target).removeClass('cmsmsColumnHover');
					}, 
					drop : function (event, ui) { 
						if (!ui.draggable.hasClass('iris-square-value')) {
							if (event.originalEvent.type === 'mouseup') {
								...
								out += '<div class="' + ui.draggable.data('shortcode') + ' cmsms_dropped hideEl">' + 
									'<div class="innerContent">' + 
										((ui.draggable.data('pair') && typeof cmsmsShortcodes[ui.draggable.data('shortcode')].visual === 'string') ? ((shcdDefNew !== '') ? shcdDefNew : shcdDef) : '<span class="cmsmsShortcodeTitle ' + cmsmsShortcodes[ui.draggable.data('shortcode')].icon + '">' + cmsmsShortcodes[ui.draggable.data('shortcode')].title + '</span>') + 
									'</div>' + 
									'<div class="innerCode">' + 
						...
						obj.methods.button_row.draggable( { 
							...
							start : function (event, ui) {
								$(ui.helper).attr('class', 'cmsms_row').css( { 
									...
									height: 75, 
									padding : '25px 0 0' 
								} ).addClass('ui-front').wrapInner('<div class="cmsms_row" />').empty().append('<div class="innerRow" />').find('.innerRow').height(63).append('<span class="cmsmsShortcodeTitle admin-icon-row" />').parent().prepend('<div class="innerHead">' + 
									'<span>' + cmsmsRow.title + '</span>' + 
								'</div>');
							} 
						} );
						...
						obj.methods.buttons.draggable( { 
							...
							start : function (event, ui) { 
								$(ui.helper).attr('class', ui.helper.data('shortcode')).css( { 
									...
									height : 90, 
									padding : '25px 0 0' 
								} ).addClass('ui-front').wrapInner('<div class="' + ui.helper.data('shortcode') + '" />').empty().append('<div class="innerHead" />').append('<div class="innerShortcode" />').parent().find('.innerShortcode').append('<div class="innerContent">' + 
