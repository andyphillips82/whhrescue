 ...
 * Custom Shortcodes
 */
function cmsms_add_custom_shortcodes() {
	$cmsms_custom_shortcodes = array();
	
	$cmsms_custom_shortcodes = apply_filters('cmsms_custom_shortcodes_filter', $cmsms_custom_shortcodes);
	
	
	if (!empty($cmsms_custom_shortcodes)) {
		foreach ($cmsms_custom_shortcodes as $cmsms_custom_shortcode) {
			add_shortcode($cmsms_custom_shortcode, $cmsms_custom_shortcode);
		}
	}
}

add_action('after_setup_theme', 'cmsms_add_custom_shortcodes');
...
function cmsms_notice($atts, $content = null) {
	...
	(($animation != '' && $animation_delay != '') ? ' data-delay="' . $animation_delay . '"' : '') . 
	'>' . "\n" . 
		(($close != '') ? '<a href="#" class="notice_close cmsms_theme_icon_cancel"></a>' : '') . 
...
function cmsms_icon_box($atts, $content = null) {
	...
	if ($box_icon_type == 'image' && $box_icon_image != '') {
		...
		$out .= '#cmsms_icon_box_' . $unique_id . (($box_type != 'cmsms_icon_heading_left') ? ':before' : '.cmsms_icon_heading_left .icon_box_heading:before') . ' { ' . 
	...
	if ($content != null) {
		if ($layout == 'hover') {
			...
			foreach ($images as $image) {
				...
				$gallery_item = '<li class="cmsms_gallery_item' . ((get_post_field('post_excerpt', strstr($image, '|', true)) != '') ? ' cmsms_caption' : '') . '">' . 
					'<figure>';
			...
			$out .= '<div id="cmsms_gallery_' . $unique_id . '" class="cmsms_gallery_wrap' . 
			(($classes != '') ? ' ' . $classes : '') . 
			'"' . 
			(($animation != '') ? ' data-animation="' . $animation . '"' : '') . 
			(($animation != '' && $animation_delay != '') ? ' data-delay="' . $animation_delay . '"' : '') . 
			...
			$out .= "<style type=\"text/css\">
				#cmsms_gallery_{$unique_id} .cmsms_gallery {
					margin:0 0 0 -{$gallery_padding}px;
				}
				
				#cmsms_gallery_{$unique_id} .cmsms_gallery .cmsms_gallery_item {
					padding:0 0 {$gallery_padding}px {$gallery_padding}px;
				}
			</style>";
			
			
			$out .= '<ul class="cmsms_gallery' . 
				(($gallery_columns != '') ? ' cmsms_' . $gallery_columns : '') . 
				' cmsms_more_items_loader' . 
			'">' . 
				$out_gallery_items . 
			'</ul>';
			...
			if ($gallery_arr != '') {
				$out .= '<div class="cmsms_more_gallery_items">' . 
					'<a href="javascript:void(0);" class="cmsms_button cmsms_gallery_items_loader">' . 
			...
			$out .= "<script type=\"text/javascript\">
				jQuery(document).ready(function () {
					(function ($) {
						if ($('#cmsms_gallery_{$unique_id}').find('.cmsms_gallery_item').length == '0') {
							return false;
						}
...
function cmsms_pricing_table_items($atts, $content = null) {
	...
	'<div class="cmsms_pricing_table' . ' ' . $price_columns . 
	(($classes != '') ? ' ' . $classes : '') . 
	'"' . 
	(($animation != '') ? ' data-animation="' . $animation . '"' : '') . 
	(($animation != '' && $animation_delay != '') ? ' data-delay="' . $animation_delay . '"' : '') . 
...
function cmsms_google_map_marker($atts, $content = null) {
	...
	$map_out .= (($content != '') ? 'html: "' . addslashes(str_replace(array("\r", "\n"), '', cmsms_divpdel($content))) . '",' : '') . 
...
function cmsms_blog($atts, $content = null) {
	...
	$out = "<div class=\"cmsms_wrap_blog entry-summary\" id=\"blog_{$unique_id}\">";
	...
	if ($pagination !== 'disabled') {
		$out .= '<div class="cmsms_wrap_more_posts">';
			...
			if ($pagination == 'pagination' && $query->max_num_pages > 1) {
				$out .= cmsms_pagination($query->max_num_pages);
			...
			<div class=\"cmsms_project_filter\">
				...
				if ($filter != '') {
					$out .= "<div class=\"cmsms_project_filter_block\">
						<a class=\"cmsms_project_filter_but cmsms_theme_icon_resp_nav button\">
							<span>" . $filter_text . "</span>
						</a>
