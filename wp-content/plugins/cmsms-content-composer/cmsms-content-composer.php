...
class Cmsms_Content_Composer { 
	function __construct() { 
		define('CMSMS_CONTENT_COMPOSER_VERSION', '1.5.7');
		
		define('CMSMS_CONTENT_COMPOSER_FILE', __FILE__);
		
		define('CMSMS_CONTENT_COMPOSER_PATH', plugin_dir_path(CMSMS_CONTENT_COMPOSER_FILE));
		
		define('CMSMS_CONTENT_COMPOSER_URL', plugin_dir_url(CMSMS_CONTENT_COMPOSER_FILE));
		...
		require_once(CMSMS_CONTENT_COMPOSER_PATH . 'inc/like/likes-posttype.php');
		
		require_once(CMSMS_CONTENT_COMPOSER_PATH . 'gutenberg/gutenberg.php');
		...
		register_activation_hook(CMSMS_CONTENT_COMPOSER_FILE, array($this, 'cmsms_content_composer_activate_deactivate'));
		
		register_deactivation_hook(CMSMS_CONTENT_COMPOSER_FILE, array($this, 'cmsms_content_composer_activate_deactivate'));
		...
		load_plugin_textdomain('cmsms_content_composer', false, dirname(plugin_basename(CMSMS_CONTENT_COMPOSER_FILE)) . '/languages/');
	}
	...
	function cmsms_composer_enqueue_scripts($hook) {
		...
		/* Start Global Translations */
			...
			'link_target' => 									__('Link Target', 'cmsms_content_composer'),
		...
		/* Start cmsms_gallery Translations */
			...
			'gallery_field_gallery_type_title' =>				__('Gallery Type', 'cmsms_content_composer'),
			'gallery_field_gallery_type_grid' =>				__('Grid Gallery', 'cmsms_content_composer'),
			'gallery_field_gallery_type_masonry' =>				__('Masonry Gallery', 'cmsms_content_composer'),
			'gallery_field_gallery_count_title' =>				__('Images Number', 'cmsms_content_composer'),
			'gallery_field_gallery_count_descr' =>				__('Amount of images that will appear each time a \'Load More\' button is clicked', 'cmsms_content_composer'),
			'gallery_field_gallery_count_descr_note' =>			__('number, if empty - show all images at once', 'cmsms_content_composer'),
			'gallery_field_gallery_padding_title' =>			__('Gallery Gap', 'cmsms_content_composer'),
			'gallery_field_gallery_padding_descr_note' =>		__('number, in pixels, no gap if empty', 'cmsms_content_composer'),
	...
	// CMSMasters Editor Shortcodes	Translations
		...
		wp_register_script('jquery-base64', CMSMS_CONTENT_COMPOSER_URL . 'framework/js/jquery.base64.min.js', array('jquery'), CMSMS_CONTENT_COMPOSER_VERSION, true);
		
		
		wp_register_script('jquery-sticky-kit', CMSMS_CONTENT_COMPOSER_URL . 'framework/js/jquery.sticky-kit.min.js', array('jquery'), CMSMS_CONTENT_COMPOSER_VERSION, true);
...
";
	...
	function cmsms_content_composer_activate_deactivate() {
		flush_rewrite_rules();
	}
}
